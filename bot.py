...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем параметр deep link (например: /start product_abc123 или /start category_abc123)
    product_id = None
    category_id = None
    open_catalog = False
    open_reviews = False
    ...
    if context.args and len(context.args) > 0:
        ...
        elif arg.startswith('category_'):
            category_id = arg.replace('category_', '')
        elif arg == 'catalog':
            open_catalog = True
        elif arg == 'reviews':
            open_reviews = True
    
    # Deep link: открыть конкретный товар
    if product_id:
    ...
    # Deep link: открыть категорию
    if category_id:
        webapp_url_with_category = f"{WEBAPP_URL}/catalog?collection={category_id}"
        
        keyboard = [[InlineKeyboardButton(
            text="📂 Открыть категорию",
            web_app=WebAppInfo(url=webapp_url_with_category)
        )]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await update.message.reply_text(
            f"👋 Привет, {user.first_name}!\n\n"
            "Нажмите кнопку ниже, чтобы перейти в категорию:",
            reply_markup=reply_markup
        )
        return
    
    # Deep link: открыть каталог
    if open_catalog:
        webapp_url_catalog = f"{WEBAPP_URL}/catalog"
        
        keyboard = [[InlineKeyboardButton(
            text="🛍 Открыть каталог",
            web_app=WebAppInfo(url=webapp_url_catalog)
        )]]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    # Deep link: открыть отзывы
    if open_reviews:
        webapp_url_reviews = f"{WEBAPP_URL}/reviews"
        
        keyboard = [[InlineKeyboardButton(
            text="⭐ Открыть отзывы",
            web_app=WebAppInfo(url=webapp_url_reviews)
        )]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await update.message.reply_text(
            f"👋 Привет, {user.first_name}!\n\n"
            "Нажмите кнопку ниже, чтобы посмотреть отзывы покупателей:",
            reply_markup=reply_markup
        )
        return
